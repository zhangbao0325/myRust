...
fn as_ref_nest_example() {
    ...
    let t1 = Some(t);
    test_i32(t1);
    println!("t1: {:?}, addr: {:p}", t1, &t1);


    fn test_string(mut t : Option<String>) {
        // t = Some(String::from("bbbb"));
        if let Some(x) = t.as_ref() {
            let address = x as *const String;
            println!("address2: {:p}", address);
        }
    ...
    if let Some(x) = p1.as_ref() {
        let address = x as *const String;
        println!("address1: {:p}", address);
    }

    // println!("p1: {:?}, addr: {:p}", p1, p1.as_ref());
    test_string(p1);
    // println!("p1: {:?}, addr: {:p}", p1, &p1);
    ...
    fn hello1(x: Option<i32>) -> Option<i32> {
        x.as_ref();
        x
    }
    let s = 1234;
    ...
    fn hello2<T: AsRef<str>>(x: T) {
        let y = x.as_ref();
        println!("hello2: {:?}", y);
    }

    let ss = "hello2".to_string();
    hello2(ss);
    // println!("ss: {}", ss);
